...
def main():
    ...
    inter_latch_ms = getattr(config, "INTER_LATCH_MS", 350)
    ...
    # ---------- Finish ----------
    # Follow (PD) with events disabled to straighten after last turn.
    final_follow_ms = getattr(config, "FINAL_FOLLOW_MS", 1300)
    final_follow_throttle = getattr(config, "FINAL_FOLLOW_THROTTLE", 0.35)

    # Then drive straight into the box ignoring sensors.
    final_forward_ms = getattr(config, "FINAL_FORWARD_MS", 1300)
    final_forward_throttle = getattr(config, "FINAL_FORWARD_THROTTLE", 0.35)
    ...
    # last time we saw an intersection-like pattern
    t_last_inter = ticks_ms() - 10_000

    # latched at event trigger (only used to decide whether to do TURN_APPROACH)
    last_event_was_intersection = False

    # once we trigger the final scripted action, we will finish with FINAL_FOLLOW + FINAL_FORWARD.
    finish_mode = False
    ...
    while True:
        ...
        if inter_cond:
            t_last_inter = t_now
        inter_recent = ticks_diff(t_now, t_last_inter) <= inter_latch_ms
        ...
        # Effective event type: treat anything within the latch window as "intersection".
        ...
        # ---------------- FINAL_FOLLOW ----------------
        if state == FINAL_FOLLOW:
            # Follow the line with PD, but IGNORE events completely.
            if err is None:
                ...
                motors.arcade(final_follow_throttle, steer)
                last_err = err

            if ticks_diff(t_now, state_t0) >= final_follow_ms:
                set_state(FINAL_FORWARD, "final follow done -> final forward")
            continue

        # ---------------- FINAL_FORWARD ----------------
        if state == FINAL_FORWARD:
            # Drive straight into the black box. Ignore sensors/events.
            motors.arcade(final_forward_throttle, 0.0)
            if ticks_diff(t_now, state_t0) >= final_forward_ms:
                set_state(STOP, "final forward complete")
            continue
        ...
        if state == DO_STRAIGHT:
            ...
            if elapsed >= straight_min_ms and straight_out >= inter_exit_n and good_line:
                ...
                if finish_mode and step >= len(mission):
                    set_state(FINAL_FOLLOW, "final straight complete -> final follow")
                else:
        ...
        if state == TURN_ALIGN:
            ...
            if good_line and (not inter_cond) and (sumw <= 2):
                ...
                if acquire_ok >= reacquire_n:
                    ...
                    if finish_mode and step >= len(mission):
                        set_state(FINAL_FOLLOW, "final turn complete -> final follow")
                    else:
        ...
        if state == FOLLOW:
            if step >= len(mission):
                # Safety net: if we somehow fall through here at the end,
                # still finish with the timed sequence.
                set_state(FINAL_FOLLOW, "mission complete -> final follow")
                continue
            ...
            can_detect = (
                events_armed
                and (ticks_diff(t_now, last_event) >= event_cooldown_ms)
                and (good_rearm >= event_rearm_n)
            )
            ...
            if can_detect:
                if event_cond:
                    ...
                    if event_in_count >= event_enter_n:
                        ...
                        if step == (len(mission) - 1):
                            finish_mode = True

                        last_event_was_intersection = bool(eff_inter)
