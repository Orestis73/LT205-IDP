...
from control.runtime_variables_v2 import Mem, State
from control.movement_v2 import (
...
def _build_initial_mission(nav):
    ...
    dbg("INITIAL MISSION", mission)
    return mission


def _maybe_wait_for_start(task_sensors, motors):
    # Hook for the real start button.
    # With the current stub start_pressed() returns True immediately.
    ...
    while True:
        ...
        if mem.state == states.FOLLOW:
            ...
            if step is None:
                ...
                if nav.mode == "to_place":
                    mem.dir_turn = nav.current_place_turn
                    set_state(mem, states.PLACE, "arrived at colour bay entry")
                    continue
        ...
        if mem.state == states.PLACE:
            colour = nav.stack_info[nav.active_stack]["colour"]
            place_movement(mem, motors, sensors, states, task_sensors, colour)
            if mem.state == states.FOLLOW:
